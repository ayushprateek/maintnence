...
class EditPurchaseRequest extends StatefulWidget {
  static bool saveButtonPressed = false;
  int index = 0;

  EditPurchaseRequest(int index, {this.onBackPressed}) {
    this.index = index;
  }
...
class _JobCardState extends State<EditPurchaseRequest> {
  @override
  void initState() {
    super.initState();
  }
  ...
  save() async {
    ...
    EditPurchaseRequest.saveButtonPressed = false;
    if (DataSync.isSyncing()) {
      ...
      if (!GeneralData.validate()) {
        ...
        if (!EditPurchaseRequest.saveButtonPressed) {
          EditPurchaseRequest.saveButtonPressed = true;
          showLoader(context);
