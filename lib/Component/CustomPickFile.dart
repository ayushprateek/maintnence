...
bool isFileSizeValidate({required File file}) {
  // Get the file size in bytes
  int fileSizeInBytes = file.lengthSync();
  // Convert bytes to megabytes
  double fileSizeInMB = fileSizeInBytes / (1024 * 1024);
  // Display the file size in megabytes
  print('File size: ${fileSizeInMB.toStringAsFixed(2)} MB');
  if (fileSizeInMB > 3) {
    getErrorSnackBar('Please select file less that of 3 MB');
  }
...
Future pickFile({required BuildContext context}) async {
  try {
    ...
    if (result != null) {
      ...
      if (!isFileSizeValidate(file: File(path))) {
        return '';
      }
    }
...
showUpdateAppAlertDialog() {
  showDialogConfirmationCustomButton(
    message: 'Yo are using old version of app.Please update to continue.',
    title: 'Update Alert',
