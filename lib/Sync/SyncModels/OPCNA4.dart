...
List<OPCNA4> oPCNA4FromJson(String str) => List<OPCNA4>.from(
    json.decode(str).map((x) => OPCNA4.fromJson(x)));
String oPCNA4ToJson(List<OPCNA4> data) =>
    json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
Future<List<OPCNA4>> dataSyncOPCNA4() async {
  var res = await http.get(headers: header, Uri.parse(prefix + "OPCNA4" + postfix));
  print(res.body);
...
Future<void> updateOPCNA4(int id, Map<String, dynamic> values, BuildContext context) async {
...
Future<List<OPCNA4>> retrieveOPCNA4ById(BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult = await db.query('OPCNA4', where: str, whereArgs: l);
  return queryResult.map((e) => OPCNA4.fromJson(e)).toList();
}
Future<String> insertOPCNA4ToServer(BuildContext? context, {String? TransId, int? id}) async {
  String response = "";
  List<OPCNA4> list = await retrieveOPCNA4ById(context, TransId == null ? DataSync.getInsertToServerStr() : "TransId = ? AND ID = ?", TransId == null ? DataSync.getInsertToServerList() : [TransId, id]);
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "OPCNA4/Add"), headers: header, body: jsonEncode(list[0].toJson()));
    response = res.body;
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http.post(Uri.parse(prefix + "OPCNA4/Add"), headers: header,
  ...
  return response;}
Future<void> updateOPCNA4OnServer(BuildContext? context, {String? condition, List? l}) async {
  List<OPCNA4> list = await retrieveOPCNA4ById(context, l == null ? DataSync.getUpdateOnServerStr() : condition ?? "", l == null ? DataSync.getUpdateOnServerList() : l);
  print(list);
  int i = 0;
  bool sentSuccessInServer = false;
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http.put(Uri.parse(prefix + 'OPCNA4/Update'), headers: header, body: jsonEncode(map)).timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
      print(await res.statusCode);
      if (res.statusCode == 201 || res.statusCode == 500) {
