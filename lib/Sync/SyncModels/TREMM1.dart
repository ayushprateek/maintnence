...
List<TREMM1> tREMM1FromJson(String str) =>
    List<TREMM1>.from(json.decode(str).map((x) => TREMM1.fromJson(x)));
...
Future<List<TREMM1>> dataSyncTREMM1() async {
  var res =
      await http.get(headers: header, Uri.parse(prefix + "TREMM1" + postfix));
  print(res.body);
...
Future<void> insertTREMM1(Database db, {List? list}) async {
  ...
  var v = await db.rawQuery('''
    SELECT T0.*
FROM TREMM1_Temp T0
LEFT JOIN TREMM1 T1 ON T0.Code = T1.Code AND T0.RowId = T1.RowId
...
Future<List<TREMM1>> retrieveTREMM1ById(
    BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult =
      await db.query('TREMM1', where: str, whereArgs: l);
  return queryResult.map((e) => TREMM1.fromJson(e)).toList();
}

// Future<String> insertTREMM1ToServer(BuildContext? context,
//     {String? TransId, int? id}) async {
//   String response = "";
//   List<TREMM1> list = await retrieveTREMM1ById(
//       context,
//       TransId == null
...
//   if (TransId != null) {
//     list[0].ID = 0;
//     var res = await http.post(Uri.parse(prefix + "TREMM1/Add"),
...
//         map.remove('ID');
//         var res = await http
//             .post(Uri.parse(prefix + "TREMM1/Add"),
...
//             map["has_created"] = 0;
//             var x = await db.update("TREMM1", map,
...
// }
//
// Future<void> updateTREMM1OnServer(BuildContext? context,
...
//       Map<String, dynamic> map = list[i].toJson();
//       var res = await http
//           .put(Uri.parse(prefix + 'TREMM1/Update'),
//               headers: header, body: jsonEncode(map))
//           .timeout(Duration(seconds: 30), onTimeout: () {
//         return http.Response('Error', 500);
//       });
