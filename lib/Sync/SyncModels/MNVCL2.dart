...
List<MNVCL2> mNVCL2FromJson(String str) => List<MNVCL2>.from(
    json.decode(str).map((x) => MNVCL2.fromJson(x)));
String mNVCL2ToJson(List<MNVCL2> data) =>
    json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
Future<List<MNVCL2>> dataSyncMNVCL2() async {
  var res = await http.get(headers: header, Uri.parse(prefix + "MNVCL2" + postfix));
  print(res.body);
...
Future<void> updateMNVCL2(int id, Map<String, dynamic> values, BuildContext context) async {
...
Future<List<MNVCL2>> retrieveMNVCL2ById(BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult = await db.query('MNVCL2', where: str, whereArgs: l);
  return queryResult.map((e) => MNVCL2.fromJson(e)).toList();
}
Future<String> insertMNVCL2ToServer(BuildContext? context, {String? TransId, int? id}) async {
  String response = "";
  List<MNVCL2> list = await retrieveMNVCL2ById(context, TransId == null ? DataSync.getInsertToServerStr() : "TransId = ? AND ID = ?", TransId == null ? DataSync.getInsertToServerList() : [TransId, id]);
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "MNVCL2/Add"), headers: header, body: jsonEncode(list[0].toJson()));
    response = res.body;
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http.post(Uri.parse(prefix + "MNVCL2/Add"), headers: header,
  ...
  return response;}
Future<void> updateMNVCL2OnServer(BuildContext? context, {String? condition, List? l}) async {
  List<MNVCL2> list = await retrieveMNVCL2ById(context, l == null ? DataSync.getUpdateOnServerStr() : condition ?? "", l == null ? DataSync.getUpdateOnServerList() : l);
  print(list);
  int i = 0;
  bool sentSuccessInServer = false;
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http.put(Uri.parse(prefix + 'MNVCL2/Update'), headers: header, body: jsonEncode(map)).timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
      print(await res.statusCode);
      if (res.statusCode == 201 || res.statusCode == 500) {
