...
List<OPOCNA> oPOCNAFromJson(String str) =>
    List<OPOCNA>.from(json.decode(str).map((x) => OPOCNA.fromJson(x)));
...
Future<List<OPOCNA>> dataSyncOPOCNA() async {
  var res =
      await http.get(headers: header, Uri.parse(prefix + "OPOCNA" + postfix));
  print(res.body);
...
Future<List<OPOCNA>> retrieveOPOCNAById(
    BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult =
      await db.query('OPOCNA', where: str, whereArgs: l);
  return queryResult.map((e) => OPOCNA.fromJson(e)).toList();
}

// Future<String> insertOPOCNAToServer(BuildContext? context,
//     {String? TransId, int? id}) async {
//   String response = "";
//   List<OPOCNA> list = await retrieveOPOCNAById(
//       context,
//       TransId == null
...
//   if (TransId != null) {
//     list[0].ID = 0;
//     var res = await http.post(Uri.parse(prefix + "OPOCNA/Add"),
...
//         map.remove('ID');
//         var res = await http
//             .post(Uri.parse(prefix + "OPOCNA/Add"),
...
//             map["has_created"] = 0;
//             var x = await db.update("OPOCNA", map,
...
// }
//
// Future<void> updateOPOCNAOnServer(BuildContext? context,
...
//       Map<String, dynamic> map = list[i].toJson();
//       var res = await http
//           .put(Uri.parse(prefix + 'OPOCNA/Update'),
//               headers: header, body: jsonEncode(map))
//           .timeout(Duration(seconds: 30), onTimeout: () {
//         return http.Response('Error', 500);
//       });
