...
List<MNOEQG> mNOEQGFromJson(String str) =>
    List<MNOEQG>.from(json.decode(str).map((x) => MNOEQG.fromJson(x)));
...
Future<List<MNOEQG>> dataSyncMNOEQG() async {
  var res =
      await http.get(headers: header, Uri.parse(prefix + "MNOEQG" + postfix));
  print(res.body);
...
Future<List<MNOEQG>> retrieveMNOEQGById(
    BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult =
      await db.query('MNOEQG', where: str, whereArgs: l);
  return queryResult.map((e) => MNOEQG.fromJson(e)).toList();
}

Future<String> insertMNOEQGToServer(BuildContext? context,
    {String? TransId, int? id}) async {
  String response = "";
  List<MNOEQG> list = await retrieveMNOEQGById(
      context,
      TransId == null
  ...
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "MNOEQG/Add"),
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http
            .post(Uri.parse(prefix + "MNOEQG/Add"),
        ...
        if (res.statusCode == 409) {
          ...
          final Database db = await initializeDB(context);
          MNOEQG model = MNOEQG.fromJson(jsonDecode(res.body));
          map["ID"] = model.ID;
          map["has_created"] = 0;
          var x = await db.update("MNOEQG", map,
...
Future<void> updateMNOEQGOnServer(BuildContext? context,
  ...
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http
          .put(Uri.parse(prefix + 'MNOEQG/Update'),
              headers: header, body: jsonEncode(map))
          .timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
