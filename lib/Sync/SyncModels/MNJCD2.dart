...
Future<void> insertMNJCD2(Database db, {List? list}) async {
  ...
  if (list != null) {
    ...
    customers = await dataSyncMNJCD2();
  }
  print(customers);
  ...
  for (var i = 0; i < customers.length; i += batchSize) {
    ...
    await db.transaction((txn) async {
      var batch = txn.batch();
      for (MNJCD2 record in batchRecords) {
        try {
          batch.insert('MNJCD2_Temp', record.toJson());
        } catch (e) {
          writeToLogFile(
              text: e.toString(),
  ...
  stopwatch.stop();
  print('Time taken for MNJCD2 update: ${stopwatch.elapsedMilliseconds}ms');
  stopwatch.reset();
  stopwatch.start();
  // var v = await db.rawQuery("Select * from MNJCD2_Temp where TransId not in (Select TransId from MNJCD2)");
  var v = await db.rawQuery('''
    SELECT T0.*
FROM MNJCD2_Temp T0
LEFT JOIN MNJCD2 T1 ON T0.TransId = T1.TransId 
...
''');
  ...
  stopwatch.stop();
  print(
      'Time taken for MNJCD2_Temp and MNJCD2 compare : ${stopwatch.elapsedMilliseconds}ms');
  stopwatch.reset();
  print('Time taken for insertDataInTable: ${stopwatch.elapsedMilliseconds}ms');
  // stopwatch.start();
  // // await batch3.commit(noResult: true);
  await db.delete('MNJCD2_Temp');
}

Future<List<MNJCD2>> retrieveMNJCD2(BuildContext context) async {
    ...
    do {
      ...
      try {
        ...
        if (res.statusCode == 409) {
          ...
          final Database db = await initializeDB(context);
          MNJCD2 model = MNJCD2.fromJson(jsonDecode(res.body));
          map["ID"] = model.ID;
          map["has_created"] = 0;
