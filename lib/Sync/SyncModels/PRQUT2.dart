...
List<PRQUT2> pRQUT2FromJson(String str) => List<PRQUT2>.from(
    json.decode(str).map((x) => PRQUT2.fromJson(x)));
String pRQUT2ToJson(List<PRQUT2> data) =>
    json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
Future<List<PRQUT2>> dataSyncPRQUT2() async {
  var res = await http.get(headers: header, Uri.parse(prefix + "PRQUT2" + postfix));
  print(res.body);
...
Future<void> updatePRQUT2(int id, Map<String, dynamic> values, BuildContext context) async {
...
Future<List<PRQUT2>> retrievePRQUT2ById(BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult = await db.query('PRQUT2', where: str, whereArgs: l);
  return queryResult.map((e) => PRQUT2.fromJson(e)).toList();
}
Future<String> insertPRQUT2ToServer(BuildContext? context, {String? TransId, int? id}) async {
  String response = "";
  List<PRQUT2> list = await retrievePRQUT2ById(context, TransId == null ? DataSync.getInsertToServerStr() : "TransId = ? AND ID = ?", TransId == null ? DataSync.getInsertToServerList() : [TransId, id]);
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "PRQUT2/Add"), headers: header, body: jsonEncode(list[0].toJson()));
    response = res.body;
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http.post(Uri.parse(prefix + "PRQUT2/Add"), headers: header,
        ...
        if (res.statusCode == 201 || res.statusCode == 500) {
          ...
          if (res.statusCode == 201) {
            ...
            map["has_created"] = 0;
            var x = await db.update("PRQUT2", map, where: "TransId = ? AND RowId = ?", whereArgs: [map["TransId"], map["RowId"]]);
            print(x.toString());}}
        print(res.body);
      } catch (e) {
  ...
  return response;}
Future<void> updatePRQUT2OnServer(BuildContext? context, {String? condition, List? l}) async {
  List<PRQUT2> list = await retrievePRQUT2ById(context, l == null ? DataSync.getUpdateOnServerStr() : condition ?? "", l == null ? DataSync.getUpdateOnServerList() : l);
  print(list);
  int i = 0;
  bool sentSuccessInServer = false;
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http.put(Uri.parse(prefix + 'PRQUT2/Update'), headers: header, body: jsonEncode(map)).timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
      print(await res.statusCode);
      if (res.statusCode == 201 || res.statusCode == 500) {
