...
List<PROPDN> pROPDNFromJson(String str) =>
    List<PROPDN>.from(json.decode(str).map((x) => PROPDN.fromJson(x)));
...
Future<List<PROPDN>> dataSyncPROPDN() async {
  var res =
      await http.get(headers: header, Uri.parse(prefix + "PROPDN" + postfix));
  print(res.body);
...
Future<List<PROPDN>> retrievePROPDNForSearch({
  ...
  final List<Map<String, Object?>> queryResult =
      await db.rawQuery("SELECT * FROM PROPDN WHERE TransId LIKE '$query'");
...
Future<List<PROPDN>> retrievePROPDNById(
    BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult =
      await db.query('PROPDN', where: str, whereArgs: l);
  return queryResult.map((e) => PROPDN.fromJson(e)).toList();
}

Future<String> insertPROPDNToServer(BuildContext? context,
    {String? TransId, int? id}) async {
  String response = "";
  List<PROPDN> list = await retrievePROPDNById(
      context,
      TransId == null
  ...
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "PROPDN/Add"),
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http
            .post(Uri.parse(prefix + "PROPDN/Add"),
...
Future<void> updatePROPDNOnServer(BuildContext? context,
  ...
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http
          .put(Uri.parse(prefix + 'PROPDN/Update'),
              headers: header, body: jsonEncode(map))
          .timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
