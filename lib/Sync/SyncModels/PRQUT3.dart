...
Future<void> insertPRQUT3(Database db, {List? list}) async {
  ...
  if (list != null) {
    ...
    customers = await dataSyncPRQUT3();
  }
  print(customers);
  ...
  for (var i = 0; i < customers.length; i += batchSize) {
    ...
    await db.transaction((txn) async {
      var batch = txn.batch();
      for (PRQUT3 record in batchRecords) {
        try {
          batch.insert('PRQUT3_Temp', record.toJson());
        } catch (e) {
          writeToLogFile(
              text: e.toString(),
  ...
  stopwatch.stop();
  print('Time taken for PRQUT3 update: ${stopwatch.elapsedMilliseconds}ms');
  stopwatch.reset();
  stopwatch.start();
  // var v = await db.rawQuery("Select * from PRQUT3_Temp where TransId not in (Select TransId from PRQUT3)");
  var v = await db.rawQuery('''
    SELECT T0.*
FROM PRQUT3_Temp T0
LEFT JOIN PRQUT3 T1 ON T0.TransId = T1.TransId 
...
''');
  ...
  stopwatch.stop();
  print(
      'Time taken for PRQUT3_Temp and PRQUT3 compare : ${stopwatch.elapsedMilliseconds}ms');
  stopwatch.reset();
  print('Time taken for insertDataInTable: ${stopwatch.elapsedMilliseconds}ms');
  // stopwatch.start();
  // // await batch3.commit(noResult: true);
  await db.delete('PRQUT3_Temp');
}

Future<List<PRQUT3>> retrievePRQUT3(BuildContext context) async {
