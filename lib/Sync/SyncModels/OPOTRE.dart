...
List<OPOTRE> oPOTREFromJson(String str) =>
    List<OPOTRE>.from(json.decode(str).map((x) => OPOTRE.fromJson(x)));
...
Future<List<OPOTRE>> dataSyncOPOTRE() async {
  var res =
      await http.get(headers: header, Uri.parse(prefix + "OPOTRE" + postfix));
  print(res.body);
...
Future<List<OPOTRE>> retrieveOPOTREById(
    BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult =
      await db.query('OPOTRE', where: str, whereArgs: l);
  return queryResult.map((e) => OPOTRE.fromJson(e)).toList();
}

// Future<String> insertOPOTREToServer(BuildContext? context,
//     {String? TransId, int? id}) async {
//   String response = "";
//   List<OPOTRE> list = await retrieveOPOTREById(
//       context,
//       TransId == null
...
//   if (TransId != null) {
//     list[0].ID = 0;
//     var res = await http.post(Uri.parse(prefix + "OPOTRE/Add"),
...
//         map.remove('ID');
//         var res = await http
//             .post(Uri.parse(prefix + "OPOTRE/Add"),
...
//             map["has_created"] = 0;
//             var x = await db.update("OPOTRE", map,
...
// }
//
// Future<void> updateOPOTREOnServer(BuildContext? context,
...
//       Map<String, dynamic> map = list[i].toJson();
//       var res = await http
//           .put(Uri.parse(prefix + 'OPOTRE/Update'),
//               headers: header, body: jsonEncode(map))
//           .timeout(Duration(seconds: 30), onTimeout: () {
//         return http.Response('Error', 500);
//       });
