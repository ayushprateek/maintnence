...
Future<void> insertOPCNA3(Database db, {List? list}) async {
  ...
  if (list != null) {
    ...
    customers = await dataSyncOPCNA3();
  }
  print(customers);
  ...
  for (var i = 0; i < customers.length; i += batchSize) {
    ...
    await db.transaction((txn) async {
      var batch = txn.batch();
      for (OPCNA3 record in batchRecords) {
        try {
          batch.insert('OPCNA3_Temp', record.toJson());
        } catch (e) {
          writeToLogFile(
              text: e.toString(),
  ...
  stopwatch.stop();
  print('Time taken for OPCNA3 update: ${stopwatch.elapsedMilliseconds}ms');
  stopwatch.reset();
  stopwatch.start();
  // var v = await db.rawQuery("Select * from OPCNA3_Temp where TransId not in (Select TransId from OPCNA3)");
  var v = await db.rawQuery('''
    SELECT T0.*
FROM OPCNA3_Temp T0
LEFT JOIN OPCNA3 T1 ON T0.TransId = T1.TransId 
...
''');
  ...
  stopwatch.stop();
  print(
      'Time taken for OPCNA3_Temp and OPCNA3 compare : ${stopwatch.elapsedMilliseconds}ms');
  stopwatch.reset();
  print('Time taken for insertDataInTable: ${stopwatch.elapsedMilliseconds}ms');
  // stopwatch.start();
  // // await batch3.commit(noResult: true);
  await db.delete('OPCNA3_Temp');
}

Future<List<OPCNA3>> retrieveOPCNA3(BuildContext context) async {
