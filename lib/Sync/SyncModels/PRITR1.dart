...
List<PRITR1> pRITR1FromJson(String str) =>
    List<PRITR1>.from(json.decode(str).map((x) => PRITR1.fromJson(x)));
...
Future<List<PRITR1>> dataSyncPRITR1() async {
  var res =
      await http.get(headers: header, Uri.parse(prefix + "PRITR1" + postfix));
  print(res.body);
...
Future<List<PRITR1>> retrievePRITR1ById(
    BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult =
      await db.query('PRITR1', where: str, whereArgs: l);
  return queryResult.map((e) => PRITR1.fromJson(e)).toList();
}

Future<String> insertPRITR1ToServer(BuildContext? context,
    {String? TransId, int? id}) async {
  String response = "";
  List<PRITR1> list = await retrievePRITR1ById(
      context,
      TransId == null
  ...
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "PRITR1/Add"),
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http
            .post(Uri.parse(prefix + "PRITR1/Add"),
        ...
        if (res.statusCode == 409) {
          ...
          final Database db = await initializeDB(context);
          PRITR1 model = PRITR1.fromJson(jsonDecode(res.body));
          map["ID"] = model.ID;
          map["has_created"] = 0;
          var x = await db.update("PRITR1", map,
...
Future<void> updatePRITR1OnServer(BuildContext? context,
  ...
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http
          .put(Uri.parse(prefix + 'PRITR1/Update'),
              headers: header, body: jsonEncode(map))
          .timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
