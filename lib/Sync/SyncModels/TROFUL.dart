...
List<TROFUL> tROFULFromJson(String str) => List<TROFUL>.from(
    json.decode(str).map((x) => TROFUL.fromJson(x)));
String tROFULToJson(List<TROFUL> data) =>
    json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
Future<List<TROFUL>> dataSyncTROFUL() async {
  var res = await http.get(headers: header, Uri.parse(prefix + "TROFUL" + postfix));
  print(res.body);
...
Future<void> updateTROFUL(int id, Map<String, dynamic> values, BuildContext context) async {
...
Future<List<TROFUL>> retrieveTROFULById(BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult = await db.query('TROFUL', where: str, whereArgs: l);
  return queryResult.map((e) => TROFUL.fromJson(e)).toList();
}
Future<String> insertTROFULToServer(BuildContext? context, {String? TransId, int? id}) async {
  String response = "";
  List<TROFUL> list = await retrieveTROFULById(context, TransId == null ? DataSync.getInsertToServerStr() : "TransId = ? AND ID = ?", TransId == null ? DataSync.getInsertToServerList() : [TransId, id]);
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "TROFUL/Add"), headers: header, body: jsonEncode(list[0].toJson()));
    response = res.body;
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http.post(Uri.parse(prefix + "TROFUL/Add"), headers: header,
  ...
  return response;}
Future<void> updateTROFULOnServer(BuildContext? context, {String? condition, List? l}) async {
  List<TROFUL> list = await retrieveTROFULById(context, l == null ? DataSync.getUpdateOnServerStr() : condition ?? "", l == null ? DataSync.getUpdateOnServerList() : l);
  print(list);
  int i = 0;
  bool sentSuccessInServer = false;
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http.put(Uri.parse(prefix + 'TROFUL/Update'), headers: header, body: jsonEncode(map)).timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
      print(await res.statusCode);
      if (res.statusCode == 201 || res.statusCode == 500) {
