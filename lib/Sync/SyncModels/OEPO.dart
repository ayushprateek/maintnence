...
Future<void> insertOEPO(Database db, {List? list}) async {
  ...
  for (var i = 0; i < customers.length; i += batchSize) {
    ...
    await db.transaction((txn) async {
      var batch = txn.batch();
      for (OEPO record in batchRecords) {
        try {
          batch.insert('OEPO_Temp', record.toJson());
        } catch (e) {
          writeToLogFile(
              text: e.toString(),
  ...
  stopwatch.stop();
  print('Time taken for OEPO update: ${stopwatch.elapsedMilliseconds}ms');
  stopwatch.reset();
  stopwatch.start();
  // var v = await db.rawQuery("Select * from OEPO_Temp where TransId not in (Select TransId from OEPO)");
  var v = await db.rawQuery('''
    SELECT T0.*
FROM OEPO_Temp T0
LEFT JOIN OEPO T1 ON T0.ID = T1.ID 
WHERE T1.ID IS NULL;
''');
  ...
  stopwatch.stop();
  print(
      'Time taken for OEPO_Temp and OEPO compare : ${stopwatch.elapsedMilliseconds}ms');
  stopwatch.reset();
  print('Time taken for insertDataInTable: ${stopwatch.elapsedMilliseconds}ms');
  // stopwatch.start();
  // // await batch3.commit(noResult: true);
  await db.delete('OEPO_Temp');
}

Future<List<OEPO>> retrieveOEPO(BuildContext context) async {
...
//   String response = "";
//   List<OEPO> list = await retrieveOEPOById(context, TransId == null ? DataSync.getInsertToServerStr() : "TransId = ? AND ID = ?", TransId == null ? DataSync.getInsertToServerList() : [TransId, id]);
//   if (TransId != null) {
//     list[0].ID = 0;
//     var res = await http.post(Uri.parse(prefix + "OEPO/Add"), headers: header, body: jsonEncode(list[0].toJson()));
//     response = res.body;
...
//         map.remove('ID');
//         var res = await http.post(Uri.parse(prefix + "OEPO/Add"), headers: header,
...
//             map["has_created"] = 0;
//             var x = await db.update("OEPO", map, where: "ID = ?", whereArgs: [map["ID"]]);
//             print(x.toString());}}
//         print(res.body);
//       } catch (e) {
...
//   return response;}
// Future<void> updateOEPOOnServer(BuildContext? context, {String? condition, List? l}) async {
//   List<OEPO> list = await retrieveOEPOById(context, l == null ? DataSync.getUpdateOnServerStr() : condition ?? "", l == null ? DataSync.getUpdateOnServerList() : l);
//   print(list);
//   int i = 0;
//   bool sentSuccessInServer = false;
//   do {
...
//     try {
//       Map<String, dynamic> map = list[i].toJson();
//       var res = await http.put(Uri.parse(prefix + 'OEPO/Update'), headers: header, body: jsonEncode(map)).timeout(Duration(seconds: 30), onTimeout: () {
//         return http.Response('Error', 500);
//       });
//       print(await res.statusCode);
//       if (res.statusCode == 201 || res.statusCode == 500) {
