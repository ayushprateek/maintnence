...
List<MNCLM1> mNCLM1FromJson(String str) =>
    List<MNCLM1>.from(json.decode(str).map((x) => MNCLM1.fromJson(x)));
...
Future<List<MNCLM1>> dataSyncMNCLM1() async {
  var res =
      await http.get(headers: header, Uri.parse(prefix + "MNCLM1" + postfix));
  print(res.body);
...
Future<List<MNCLM1>> retrieveMNCLM1ById(
    BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult =
      await db.query('MNCLM1', where: str, whereArgs: l);
  return queryResult.map((e) => MNCLM1.fromJson(e)).toList();
}

// Future<String> insertMNCLM1ToServer(BuildContext? context,
//     {String? TransId, int? id}) async {
//   String response = "";
//   List<MNCLM1> list = await retrieveMNCLM1ById(
//       context,
//       TransId == null
...
//   if (TransId != null) {
//     list[0].ID = 0;
//     var res = await http.post(Uri.parse(prefix + "MNCLM1/Add"),
...
//         map.remove('ID');
//         var res = await http
//             .post(Uri.parse(prefix + "MNCLM1/Add"),
...
//             map["has_created"] = 0;
//             var x = await db.update("MNCLM1", map,
...
// }
//
// Future<void> updateMNCLM1OnServer(BuildContext? context,
...
//       Map<String, dynamic> map = list[i].toJson();
//       var res = await http
//           .put(Uri.parse(prefix + 'MNCLM1/Update'),
//               headers: header, body: jsonEncode(map))
//           .timeout(Duration(seconds: 30), onTimeout: () {
//         return http.Response('Error', 500);
//       });
