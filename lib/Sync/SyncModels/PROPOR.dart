...
List<PROPOR> pROPORFromJson(String str) => List<PROPOR>.from(
    json.decode(str).map((x) => PROPOR.fromJson(x)));
String pROPORToJson(List<PROPOR> data) =>
    json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
Future<List<PROPOR>> dataSyncPROPOR() async {
  var res = await http.get(headers: header, Uri.parse(prefix + "PROPOR" + postfix));
  print(res.body);
...
Future<void> updatePROPOR(int id, Map<String, dynamic> values, BuildContext context) async {
...
Future<List<PROPOR>> retrievePROPORById(BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult = await db.query('PROPOR', where: str, whereArgs: l);
  return queryResult.map((e) => PROPOR.fromJson(e)).toList();
}
Future<String> insertPROPORToServer(BuildContext? context, {String? TransId, int? id}) async {
  String response = "";
  List<PROPOR> list = await retrievePROPORById(context, TransId == null ? DataSync.getInsertToServerStr() : "TransId = ? AND ID = ?", TransId == null ? DataSync.getInsertToServerList() : [TransId, id]);
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "PROPOR/Add"), headers: header, body: jsonEncode(list[0].toJson()));
    response = res.body;
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http.post(Uri.parse(prefix + "PROPOR/Add"), headers: header,
        ...
        if (res.statusCode == 201 || res.statusCode == 500) {
          ...
          if (res.statusCode == 201) {
            ...
            map["has_created"] = 0;
            var x = await db.update("PROPOR", map, where: "TransId = ? AND RowId = ?", whereArgs: [map["TransId"], map["RowId"]]);
            print(x.toString());}}
        print(res.body);
      } catch (e) {
  ...
  return response;}
Future<void> updatePROPOROnServer(BuildContext? context, {String? condition, List? l}) async {
  List<PROPOR> list = await retrievePROPORById(context, l == null ? DataSync.getUpdateOnServerStr() : condition ?? "", l == null ? DataSync.getUpdateOnServerList() : l);
  print(list);
  int i = 0;
  bool sentSuccessInServer = false;
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http.put(Uri.parse(prefix + 'PROPOR/Update'), headers: header, body: jsonEncode(map)).timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
      print(await res.statusCode);
      if (res.statusCode == 201 || res.statusCode == 500) {
