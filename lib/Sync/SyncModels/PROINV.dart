...
class PROINV{
  ...
  factory PROINV.fromJson(Map<String,dynamic> json)=>PROINV(
    ...
    IsCashReceipt : json['IsCashReceipt'] is bool ? json['IsCashReceipt'] : json['IsCashReceipt']==1,
...
List<PROINV> pROINVFromJson(String str) => List<PROINV>.from(
    json.decode(str).map((x) => PROINV.fromJson(x)));
String pROINVToJson(List<PROINV> data) =>
    json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
Future<List<PROINV>> dataSyncPROINV() async {
  var res = await http.get(headers: header, Uri.parse(prefix + "PROINV" + postfix));
  print(res.body);
...
Future<void> updatePROINV(int id, Map<String, dynamic> values, BuildContext context) async {
...
Future<List<PROINV>> retrievePROINVById(BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult = await db.query('PROINV', where: str, whereArgs: l);
  return queryResult.map((e) => PROINV.fromJson(e)).toList();
}
Future<String> insertPROINVToServer(BuildContext? context, {String? TransId, int? id}) async {
  String response = "";
  List<PROINV> list = await retrievePROINVById(context, TransId == null ? DataSync.getInsertToServerStr() : "TransId = ? AND ID = ?", TransId == null ? DataSync.getInsertToServerList() : [TransId, id]);
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "PROINV/Add"), headers: header, body: jsonEncode(list[0].toJson()));
    response = res.body;
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http.post(Uri.parse(prefix + "PROINV/Add"), headers: header,
        ...
        if (res.statusCode == 201 || res.statusCode == 500) {
          ...
          if (res.statusCode == 201) {
            ...
            map["has_created"] = 0;
            var x = await db.update("PROINV", map, where: "TransId = ? AND RowId = ?", whereArgs: [map["TransId"], map["RowId"]]);
            print(x.toString());}}
        print(res.body);
      } catch (e) {
  ...
  return response;}
Future<void> updatePROINVOnServer(BuildContext? context, {String? condition, List? l}) async {
  List<PROINV> list = await retrievePROINVById(context, l == null ? DataSync.getUpdateOnServerStr() : condition ?? "", l == null ? DataSync.getUpdateOnServerList() : l);
  print(list);
  int i = 0;
  bool sentSuccessInServer = false;
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http.put(Uri.parse(prefix + 'PROINV/Update'), headers: header, body: jsonEncode(map)).timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
      print(await res.statusCode);
      if (res.statusCode == 201 || res.statusCode == 500) {
