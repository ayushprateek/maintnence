...
List<OPTRE1> oPTRE1FromJson(String str) =>
    List<OPTRE1>.from(json.decode(str).map((x) => OPTRE1.fromJson(x)));
...
Future<List<OPTRE1>> dataSyncOPTRE1() async {
  var res =
      await http.get(headers: header, Uri.parse(prefix + "OPTRE1" + postfix));
  print(res.body);
...
Future<List<OPTRE1>> retrieveOPTRE1ById(
    BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult =
      await db.query('OPTRE1', where: str, whereArgs: l);
  return queryResult.map((e) => OPTRE1.fromJson(e)).toList();
}

// Future<String> insertOPTRE1ToServer(BuildContext? context,
//     {String? TransId, int? id}) async {
//   String response = "";
//   List<OPTRE1> list = await retrieveOPTRE1ById(
//       context,
//       TransId == null
...
//   if (TransId != null) {
//     list[0].ID = 0;
//     var res = await http.post(Uri.parse(prefix + "OPTRE1/Add"),
...
//         map.remove('ID');
//         var res = await http
//             .post(Uri.parse(prefix + "OPTRE1/Add"),
...
//             map["has_created"] = 0;
//             var x = await db.update("OPTRE1", map,
...
// }
//
// Future<void> updateOPTRE1OnServer(BuildContext? context,
...
//       Map<String, dynamic> map = list[i].toJson();
//       var res = await http
//           .put(Uri.parse(prefix + 'OPTRE1/Update'),
//               headers: header, body: jsonEncode(map))
//           .timeout(Duration(seconds: 30), onTimeout: () {
//         return http.Response('Error', 500);
//       });
