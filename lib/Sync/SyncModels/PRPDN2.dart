...
List<PRPDN2> pRPDN2FromJson(String str) => List<PRPDN2>.from(
    json.decode(str).map((x) => PRPDN2.fromJson(x)));
String pRPDN2ToJson(List<PRPDN2> data) =>
    json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
Future<List<PRPDN2>> dataSyncPRPDN2() async {
  var res = await http.get(headers: header, Uri.parse(prefix + "PRPDN2" + postfix));
  print(res.body);
...
Future<void> updatePRPDN2(int id, Map<String, dynamic> values, BuildContext context) async {
...
Future<List<PRPDN2>> retrievePRPDN2ById(BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult = await db.query('PRPDN2', where: str, whereArgs: l);
  return queryResult.map((e) => PRPDN2.fromJson(e)).toList();
}
Future<String> insertPRPDN2ToServer(BuildContext? context, {String? TransId, int? id}) async {
  String response = "";
  List<PRPDN2> list = await retrievePRPDN2ById(context, TransId == null ? DataSync.getInsertToServerStr() : "TransId = ? AND ID = ?", TransId == null ? DataSync.getInsertToServerList() : [TransId, id]);
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "PRPDN2/Add"), headers: header, body: jsonEncode(list[0].toJson()));
    response = res.body;
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http.post(Uri.parse(prefix + "PRPDN2/Add"), headers: header,
        ...
        if (res.statusCode == 201 || res.statusCode == 500) {
          ...
          if (res.statusCode == 201) {
            ...
            map["has_created"] = 0;
            var x = await db.update("PRPDN2", map, where: "TransId = ? AND RowId = ?", whereArgs: [map["TransId"], map["RowId"]]);
            print(x.toString());}}
        print(res.body);
      } catch (e) {
  ...
  return response;}
Future<void> updatePRPDN2OnServer(BuildContext? context, {String? condition, List? l}) async {
  List<PRPDN2> list = await retrievePRPDN2ById(context, l == null ? DataSync.getUpdateOnServerStr() : condition ?? "", l == null ? DataSync.getUpdateOnServerList() : l);
  print(list);
  int i = 0;
  bool sentSuccessInServer = false;
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http.put(Uri.parse(prefix + 'PRPDN2/Update'), headers: header, body: jsonEncode(map)).timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
      print(await res.statusCode);
      if (res.statusCode == 201 || res.statusCode == 500) {
