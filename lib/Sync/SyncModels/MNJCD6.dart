...
List<MNJCD6> mNJCD6FromJson(String str) =>
    List<MNJCD6>.from(json.decode(str).map((x) => MNJCD6.fromJson(x)));
...
Future<List<MNJCD6>> dataSyncMNJCD6() async {
  var res =
      await http.get(headers: header, Uri.parse(prefix + "MNJCD6" + postfix));
  print(res.body);
...
Future<List<MNJCD6>> retrieveMNJCD6ById(
    BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult =
      await db.query('MNJCD6', where: str, whereArgs: l);
  return queryResult.map((e) => MNJCD6.fromJson(e)).toList();
}

Future<String> insertMNJCD6ToServer(BuildContext? context,
    {String? TransId, int? id}) async {
  String response = "";
  List<MNJCD6> list = await retrieveMNJCD6ById(
      context,
      TransId == null
  ...
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "MNJCD6/Add"),
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http
            .post(Uri.parse(prefix + "MNJCD6/Add"),
        ...
        if (res.statusCode == 201 || res.statusCode == 500) {
          ...
          if (res.statusCode == 201) {
            ...
            map["has_created"] = 0;
            var x = await db.update("MNJCD6", map,
...
Future<void> updateMNJCD6OnServer(BuildContext? context,
  ...
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http
          .put(Uri.parse(prefix + 'MNJCD6/Update'),
              headers: header, body: jsonEncode(map))
          .timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
