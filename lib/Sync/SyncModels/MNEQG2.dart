...
class MNEQG2{
  ...
  factory MNEQG2.fromJson(Map<String,dynamic> json)=>MNEQG2(
    ...
    IsTyreMaintenence : json['IsTyreMaintenence'] is bool ? json['IsTyreMaintenence'] : json['IsTyreMaintenence']==1,
  );
...
List<MNEQG2> mNEQG2FromJson(String str) => List<MNEQG2>.from(
    json.decode(str).map((x) => MNEQG2.fromJson(x)));
String mNEQG2ToJson(List<MNEQG2> data) =>
    json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
Future<List<MNEQG2>> dataSyncMNEQG2() async {
  var res = await http.get(headers: header, Uri.parse(prefix + "MNEQG2" + postfix));
  print(res.body);
...
Future<void> insertMNEQG2(Database db, {List? list}) async {
  ...
  var v = await db.rawQuery('''
    SELECT T0.*
FROM MNEQG2_Temp T0
LEFT JOIN MNEQG2 T1 ON T0.TransId = T1.TransId 
...
Future<void> updateMNEQG2(int id, Map<String, dynamic> values, BuildContext context) async {
...
Future<List<MNEQG2>> retrieveMNEQG2ById(BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult = await db.query('MNEQG2', where: str, whereArgs: l);
  return queryResult.map((e) => MNEQG2.fromJson(e)).toList();
}
Future<String> insertMNEQG2ToServer(BuildContext? context, {String? TransId, int? id}) async {
  String response = "";
  List<MNEQG2> list = await retrieveMNEQG2ById(context, TransId == null ? DataSync.getInsertToServerStr() : "TransId = ? AND ID = ?", TransId == null ? DataSync.getInsertToServerList() : [TransId, id]);
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "MNEQG2/Add"), headers: header, body: jsonEncode(list[0].toJson()));
    response = res.body;
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http.post(Uri.parse(prefix + "MNEQG2/Add"), headers: header,
        ...
        if (res.statusCode == 201 || res.statusCode == 500) {
          ...
          if (res.statusCode == 201) {
            ...
            map["has_created"] = 0;
            var x = await db.update("MNEQG2", map, where: "TransId = ? AND RowId = ?", whereArgs: [map["TransId"], map["RowId"]]);
            print(x.toString());}}
        print(res.body);
      } catch (e) {
  ...
  return response;}
Future<void> updateMNEQG2OnServer(BuildContext? context, {String? condition, List? l}) async {
  List<MNEQG2> list = await retrieveMNEQG2ById(context, l == null ? DataSync.getUpdateOnServerStr() : condition ?? "", l == null ? DataSync.getUpdateOnServerList() : l);
  print(list);
  int i = 0;
  bool sentSuccessInServer = false;
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http.put(Uri.parse(prefix + 'MNEQG2/Update'), headers: header, body: jsonEncode(map)).timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
      print(await res.statusCode);
      if (res.statusCode == 201 || res.statusCode == 500) {
