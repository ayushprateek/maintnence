...
List<MNVCL1> mNVCL1FromJson(String str) =>
    List<MNVCL1>.from(json.decode(str).map((x) => MNVCL1.fromJson(x)));
...
Future<List<MNVCL1>> dataSyncMNVCL1() async {
  var res =
      await http.get(headers: header, Uri.parse(prefix + "MNVCL1" + postfix));
  print(res.body);
...
Future<void> insertMNVCL1(Database db, {List? list}) async {
  ...
  var v = await db.rawQuery('''
    SELECT T0.*
FROM MNVCL1_Temp T0
LEFT JOIN MNVCL1 T1 ON T0.Code = T1.Code  AND T0.RowId = T1.RowId 
...
Future<List<MNVCL1>> retrieveMNVCL1ById(
    BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult =
      await db.query('MNVCL1', where: str, whereArgs: l);
  return queryResult.map((e) => MNVCL1.fromJson(e)).toList();
}

Future<String> insertMNVCL1ToServer(BuildContext? context,
    {String? TransId, int? id}) async {
  String response = "";
  List<MNVCL1> list = await retrieveMNVCL1ById(
      context,
      TransId == null
  ...
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "MNVCL1/Add"),
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http
            .post(Uri.parse(prefix + "MNVCL1/Add"),
        ...
        if (res.statusCode == 409) {
          ...
          final Database db = await initializeDB(context);
          MNVCL1 model = MNVCL1.fromJson(jsonDecode(res.body));
          map["ID"] = model.ID;
          map["has_created"] = 0;
...
Future<void> updateMNVCL1OnServer(BuildContext? context,
  ...
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http
          .put(Uri.parse(prefix + 'MNVCL1/Update'),
              headers: header, body: jsonEncode(map))
          .timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
