...
class TROEMM {
  ...
  factory TROEMM.fromJson(Map<String, dynamic> json) => TROEMM(
        ...
        FrontAxleWeight:
            double.tryParse(json['FrontAxleWeight'].toString()) ?? 0.0,
        RearAxleWeight:
            double.tryParse(json['RearAxleWeight'].toString()) ?? 0.0,
        LoadingCapacity:
            double.tryParse(json['LoadingCapacity'].toString()) ?? 0.0,
...
List<TROEMM> tROEMMFromJson(String str) =>
    List<TROEMM>.from(json.decode(str).map((x) => TROEMM.fromJson(x)));
...
Future<List<TROEMM>> dataSyncTROEMM() async {
  var res =
      await http.get(headers: header, Uri.parse(prefix + "TROEMM" + postfix));
  print(res.body);
...
Future<List<TROEMM>> retrieveTROEMMById(
    BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult =
      await db.query('TROEMM', where: str, whereArgs: l);
  return queryResult.map((e) => TROEMM.fromJson(e)).toList();
}

// Future<String> insertTROEMMToServer(BuildContext? context,
//     {String? TransId, int? id}) async {
//   String response = "";
//   List<TROEMM> list = await retrieveTROEMMById(
//       context,
//       TransId == null
...
//   if (TransId != null) {
//     list[0].ID = 0;
//     var res = await http.post(Uri.parse(prefix + "TROEMM/Add"),
...
//         map.remove('ID');
//         var res = await http
//             .post(Uri.parse(prefix + "TROEMM/Add"),
...
//             map["has_created"] = 0;
//             var x = await db.update("TROEMM", map,
...
// }
//
// Future<void> updateTROEMMOnServer(BuildContext? context,
...
//       Map<String, dynamic> map = list[i].toJson();
//       var res = await http
//           .put(Uri.parse(prefix + 'TROEMM/Update'),
//               headers: header, body: jsonEncode(map))
//           .timeout(Duration(seconds: 30), onTimeout: () {
//         return http.Response('Error', 500);
//       });
