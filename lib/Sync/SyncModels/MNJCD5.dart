...
Future<void> insertMNJCD5(Database db, {List? list}) async {
  ...
  if (list != null) {
    ...
    customers = await dataSyncMNJCD5();
  }
  print(customers);
  ...
  for (var i = 0; i < customers.length; i += batchSize) {
    ...
    await db.transaction((txn) async {
      var batch = txn.batch();
      for (MNJCD5 record in batchRecords) {
        try {
          batch.insert('MNJCD5_Temp', record.toJson());
        } catch (e) {
          writeToLogFile(
              text: e.toString(),
  ...
  stopwatch.stop();
  print('Time taken for MNJCD5 update: ${stopwatch.elapsedMilliseconds}ms');
  stopwatch.reset();
  stopwatch.start();
...
''');
  ...
  stopwatch.stop();
  print(
      'Time taken for MNJCD5_Temp and MNJCD5 compare : ${stopwatch.elapsedMilliseconds}ms');
  stopwatch.reset();
  print('Time taken for insertDataInTable: ${stopwatch.elapsedMilliseconds}ms');
  // stopwatch.start();
  // // await batch3.commit(noResult: true);
  await db.delete('MNJCD5_Temp');
}

Future<List<MNJCD5>> retrieveMNJCD5(BuildContext context) async {
