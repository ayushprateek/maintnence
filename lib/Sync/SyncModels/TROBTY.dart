...
List<TROBTY> tROBTYFromJson(String str) =>
    List<TROBTY>.from(json.decode(str).map((x) => TROBTY.fromJson(x)));
...
Future<List<TROBTY>> dataSyncTROBTY() async {
  var res =
      await http.get(headers: header, Uri.parse(prefix + "TROBTY" + postfix));
  print(res.body);
...
Future<List<TROBTY>> retrieveTROBTYById(
    BuildContext? context, String str, List l) async {
  final Database db = await initializeDB(context);
  final List<Map<String, Object?>> queryResult =
      await db.query('TROBTY', where: str, whereArgs: l);
  return queryResult.map((e) => TROBTY.fromJson(e)).toList();
}

Future<String> insertTROBTYToServer(BuildContext? context,
    {String? TransId, int? id}) async {
  String response = "";
  List<TROBTY> list = await retrieveTROBTYById(
      context,
      TransId == null
  ...
  if (TransId != null) {
    list[0].ID = 0;
    var res = await http.post(Uri.parse(prefix + "TROBTY/Add"),
    ...
    do {
      ...
      try {
        ...
        map.remove('ID');
        var res = await http
            .post(Uri.parse(prefix + "TROBTY/Add"),
        ...
        if (res.statusCode == 201 || res.statusCode == 500) {
          ...
          if (res.statusCode == 201) {
            ...
            map["has_created"] = 0;
            var x = await db.update("TROBTY", map,
...
Future<void> updateTROBTYOnServer(BuildContext? context,
  ...
  do {
    ...
    try {
      Map<String, dynamic> map = list[i].toJson();
      var res = await http
          .put(Uri.parse(prefix + 'TROBTY/Update'),
              headers: header, body: jsonEncode(map))
          .timeout(Duration(seconds: 30), onTimeout: () {
        return http.Response('Error', 500);
      });
